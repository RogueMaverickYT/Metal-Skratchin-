...
PlaySideBar::PlaySideBar() { 
	...
	mBackground = new GLTexture("DarkGray.png"); 
	mBackground->Parent(this); 
	mBackground->Scale(Vector2(3.0f, 10.0f));
	mBackground->Position(45.0f, 380.0f);

	mHighLabel = new GLTexture("HIGH", "Outrun.otf", 32, { 150, 0, 0 }); 
	mHighLabel->Parent(this); 
	mHighLabel->Position(-25.0f, 0.0f); 
	
	mScoreLabel = new GLTexture("SCORE", "Outrun.otf", 32, { 150, 0, 0 }); 
	...
	mPlayerOneLabel = new GLTexture("1UP", "Outrun.otf", 32, { 150, 0, 0 }); 
	...
	for (int i = 0; i < MAX_SHIP_TEXTURES; ++i) { 
		mShipTextures[i] = new GLTexture("DoomSlayerLogo.png", 0, 0, 60, 64); 
